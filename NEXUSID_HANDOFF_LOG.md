...
| `search.html` | Page | Project root | Dedicated player search (Nexus ID or IGN) over `nexus_players`/`nexus_player_game_accounts` — moved off `index.html`. Conformant to canonical design tokens, explicit loading state | Built 2026-09-06; rebuilt on canonical tokens 2026-09-06 (cont. 4) — not live-tested |
...
- [x] Retrofit `to-login.html`, `to-onboarding.html`, `search.html` to canonical tokens — done 2026-09-06 (see Change Log entries). Still open for: `to-dashboard.html`, `register.html`, `tournament-new.html`, `player.html` — all still use Tailwind `rounded-xl`/`2xl`/`3xl`, `#B3261E`, and raw glyph icons instead of the 2–4px radius scale / `#9B1C11` / Iconify Lucide. Conformant so far: `index.html`, `to-login.html`, `to-onboarding.html`, `search.html` (4 of 8 pages). Not started on the rest — pending Moti's call on scope/timing, or further design handoffs arriving one page at a time as they have been.
