...
**What was done:**
- Migration `0002_player_profile_additions.sql` (new, **not yet applied** — needs `supabase db push` and Moti's go-ahead before pushing, per this repo's standing caution around schema pushes): adds `nexus_tournaments.stage` (nullable free text, same convention as the existing `format` column) and a new view `nexus_match_squad_kills` (`match_id, team_id, squad_kills` = `sum(kills)` from `nexus_participations` grouped by match+team) — needed because "squad kills" isn't a stored column, it's teammates' kills summed for the same match. No new RLS policies needed; both build on tables/columns that already have public-select policies.
...
**What was NOT finished / left mid-flight:**
- Migration `0002` has **not been pushed** to the live Supabase project — needs Moti's confirmation first (schema change to `jzqmscrmeywckzodgjre`), same caution as every prior schema/config push in this log.
- Not live-browser-tested, and there's no real tournament/match data in the live project yet to sanity-check the day/match grouping or squad-kills numbers against — verified only via local static-server 200s and a manual JS-syntax check.
...
**Anything the next session needs to know before continuing:**
- Don't build anything that reads `nexus_tournaments.stage` or `nexus_match_squad_kills` against the live DB until `0002_player_profile_additions.sql` is confirmed pushed — check `supabase migration list` (or just try a query) rather than assuming.
...
| `nexus_tournaments` | DB table | Supabase | A TO's individual tournaments/events (name, game, format, stage, dates, max players) | New table, not in roadmap v1 — the roadmap only listed `nexus_participations` for the ledger, but participations need a real tournament entity to reference rather than a repeated free-text tournament name per row. Applied 2026-09-05 to the live project (`jzqmscrmeywckzodgjre`). `stage` column added 2026-09-16 (`0002_player_profile_additions.sql`) — **not yet pushed** |
...
| `nexus_match_squad_kills` | DB view | Supabase | Sum of `kills` per `(match_id, team_id)` — "how many kills did this squad get in this match." Powers the Squad Kills figure on `player.html`'s per-match tournament history | New, 2026-09-16 (`0002_player_profile_additions.sql`) — **not yet applied**, needs `supabase db push` |
...
| `player.html` | Page | Project root | Public player profile (no auth) — identity, claimed status, most-recent game account/team (+N more), 4-stat summary row, expandable per-tournament/day/match history with squad kills and map backgrounds. Search-result destination from `search.html` for player results. Conformant to canonical design tokens as of 2026-09-16 | Built 2026-09-05; live-tested in browser at the time; back-link updated 2026-09-06; fully rebuilt 2026-09-16 from the Player Profile Claude Design handoff onto canonical tokens and per-match data — not re-tested live since, and depends on `0002_player_profile_additions.sql` being pushed |
| `to.html` | Page | Project root | Public TO profile (no auth) — `to_id`, org name, IGN, optional real name, tournaments created. Search-result destination from `search.html` for organizer results. Same non-conformant Tailwind/hardcoded-hex pattern as `player.html` | Built 2026-09-16, not live-tested |
| `team.html` | Page | Project root | Public team profile (no auth) — `tm_id`, team name, active roster (links through to `player.html` per member). Search-result destination from `search.html` for team results. Same non-conformant Tailwind/hardcoded-hex pattern as `player.html` | Built 2026-09-16, not live-tested |
...
- [x] Retrofit `to-login.html`, `to-onboarding.html`, `search.html`, `player.html` to canonical tokens — `player.html` done 2026-09-16 (rebuilt from the Player Profile handoff), others done 2026-09-06 (see Change Log entries). Still open for: `to-dashboard.html`, `register.html`, `tournament-new.html`, `to.html`, `team.html` (`to.html`/`team.html` built 2026-09-16 to match `player.html`'s *old* pattern, now inconsistent with the page they were modeled on) — all still use Tailwind `rounded-xl`/`2xl`/`3xl`, `#B3261E`, and raw glyph icons instead of the 2–4px radius scale / `#9B1C11` / Iconify Lucide. Conformant so far: `index.html`, `to-login.html`, `to-onboarding.html`, `search.html`, `player.html` (5 of 10 pages). Not started on the rest — pending Moti's call on scope/timing, or further design handoffs arriving one page at a time as they have been.
- [ ] **Push `supabase/migrations/0002_player_profile_additions.sql`** (adds `nexus_tournaments.stage` + `nexus_match_squad_kills` view) to the live project via `supabase db push` — written 2026-09-16, needs Moti's go-ahead first (same caution as every prior schema/config push logged here). `player.html`'s tournament-history stage pill and squad-kills numbers will silently read as empty/wrong against the live DB until this is applied.
