...
- Migration `0003_squad_to_profile_additions.sql` (new, **not yet applied**): adds `nexus_tournaments.prize_pool` (nullable numeric, INR assumed — no currency column, matches this product's India-only scope) and a new `nexus_activity_log` table (`to_id`, `tournament_id` nullable, `event_type` check-constrained to 4 values, `detail`, `created_at`) with public-select / insert-own-only RLS, same pattern as every other TO-owned table.
...
**Anything the next session needs to know before continuing:**
- Don't build anything that reads `nexus_tournaments.prize_pool` or `nexus_activity_log` against the live DB until `0003_squad_to_profile_additions.sql` is confirmed applied — ask Moti or try a query rather than assuming, same as `0002`.
...
- [ ] **Push `supabase/migrations/0003_squad_to_profile_additions.sql`** (adds `nexus_tournaments.prize_pool` + the new `nexus_activity_log` table) to the live project — written 2026-09-16, same manual Dashboard SQL Editor paste as `0002` needed (no `supabase db push`/CLI or working MCP auth in this environment). `team.html`'s stats/history and `to.html`'s prize pool/activity feed depend on this.
