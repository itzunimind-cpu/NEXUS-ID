...
- [ ] Diagnose why the 2026-09-05 git-commit handoff-log hook doesn't reliably fire/act — missed commit `8c352e1` and, this same session, also missed `940761b` (the landing-page redesign), despite firing correctly as a no-op for unrelated `curl` commands and for the manual log-backfill commit `aec6104`. Three real-commit samples, two misses. See the 2026-09-06 Decision Log addendum.
- [ ] Retrofit the rest of NexusID's pages (`search.html`, `to-login.html`, `to-signup.html`, `to-onboarding.html`, `to-dashboard.html`, `register.html`, `tournament-new.html`, `player.html`) to the canonical METAZONE design-system tokens now placed at `metazone-main/design-system/` — specifically: replace Tailwind `rounded-xl`/`2xl`/`3xl` with the 2–4px radius scale, replace the `#B3261E` danger red with the canonical `#9B1C11`, and replace raw glyph icons (e.g. `←`) with Iconify Lucide (`<iconify-icon icon="lucide:...">`). `index.html` (rebuilt 2026-09-06) is the only page currently conformant. Not started — pending Moti's call on scope/timing.
