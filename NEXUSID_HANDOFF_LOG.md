...
**Anything the next session needs to know before continuing:**
- Don't rely on `nexus_team_members` UPDATEs (roster removal) working against the live DB until `0004_team_roster_removal.sql` is confirmed applied.
...
- [ ] **Push `supabase/migrations/0004_team_roster_removal.sql`** (adds the `nexus_team_members` UPDATE policy needed for soft-removing a roster member) to the live project — written 2026-09-16, same manual Dashboard SQL Editor paste as `0002`/`0003`. Until applied, `team-roster.html`'s Remove button will fail (no matching RLS policy yet).
